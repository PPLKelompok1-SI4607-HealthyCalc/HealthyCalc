...
<div class="max-w-4xl mx-auto px-4 py-10">
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden">
        ...
        <form method="POST" action="{{ isset($recipe) ? route('recipes.update', $recipe['id']) : route('recipes.store') }}" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @if(isset($recipe))
                @method('PUT')
            @endif
            ...
            <div>
                <label for="image" class="block text-sm font-medium text-gray-700 mb-1">Gambar Resep</label>
                <input type="file" id="image" name="image" 
                    class="w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-400">
                @if(isset($recipe) && $recipe['image'])
                    <img src="{{ asset('storage/' . $recipe['image']) }}" alt="Gambar Resep" class="mt-4 h-32 rounded-md">
                @endif
            </div>
